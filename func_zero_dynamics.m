...
dz(1) = z(2);
dz(2) = (D1 + D2*beta2)\(-D2*beta1 - H1); %referred from report 4 on canvas

dz = [dz(1), dz(2)]';

end

% function q = func_hd(s, alpha, M)
% for k=1:(M+1)
%     q = q + alpha(k)*nchoosek(M,k-1)*s^(k-1)*(1-s)^(M-(k-1));
% end
% end
% 
% function dq = func_dhd(s, alpha, M)
% for k=1:(M)
%     dq = dq + (alpha(k+1)-alpha(k))*factorial(M)/factorial(k-1)/...
%         factorial(M-(k-1)-1)*s^(k-1)*(1-s)^(M-(k-1)-1);
% end
% end